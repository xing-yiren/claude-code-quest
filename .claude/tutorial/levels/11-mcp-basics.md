...
了解 MCP（Model Context Protocol），学会判断什么时候该用 MCP 连接外部工具和服务。
...
> Game Master 注意：这一节不要在首次展示 Level 11 时一次性全部输出。只有用户通过基础任务并选择进入进阶后，才按 checkpoint 逐步披露。每次只展示一个 checkpoint，等用户完成、跳过或提问后再继续。
...
- MCP 适合三类场景：读外部上下文、查询实时系统、执行外部动作
